...
| 1 GCP 인증 | **완료** | 채널 `적운` 인증 확인. 동의 화면 프로덕션 게시 완료 |
...
| 6 `notify.py` | **구현 완료 / 실호출 미검증** | 로컬에 GH_TOKEN 이 없어 미검증. Actions 실행에서 확인된다 |
| 7 통합 점검 | **부분 완료** | 로컬 수집~업로드 완주, 중복 방지 확인. Actions 전 구간은 아직 |

### 다음에 할 것

1. **Actions → `daily-shorts-draft` → Run workflow → `count` 2.**
   이번엔 dry_run 을 끄고 돌린다. 여기서만 검증되지 않은 것이 두 가지다.
   - Task 6 의 실제 Issue 생성 (`GITHUB_TOKEN` 은 Actions 안에서만 주입된다)
   - `state/posted.json` 자동 커밋·푸시
2. 올라간 영상을 스튜디오에서 검수하고 **공개 전환은 직접** 한다.

### 2026-08-20 실행 기록

로컬에서 수집~업로드 전 구간을 완주했다.

- 대본 319자 / 309자, 원문 유사도 0.27 / 0.39 (임계 0.80)
- 음성 40.7초 / 39.8초, 영상 1080x1920 h264 30fps, 한글 자막 육안 확인
- 비공개 업로드 2건. `videos.list` 로 `privacyStatus=private` 확인
- 다음 수집에서 두 기사가 재선정되지 않는 것까지 확인 (중복 방지 동작)

### 막혀 있는 것

- **Task 6 실호출** — `GH_TOKEN` 이 로컬에 없다. Actions 실행에서 자동으로 검증된다.
